...
| **`@1shotapi/ows-credentials`** | Standardized credential work: types, OID4VCI/OID4VP client contracts, SD-JWT VC presentation assembly, status/trust hooks. No storage implementation. |
| **Host Layer** | Calls `proxy.credentials.*`; never touches credential payloads or signing keys directly. |
| **Signing Layer** | Signs digests/curves only (`signDigest` with `ed25519` for SD-JWT key binding). JWT assembly stays in `ows-credentials` / `ows-signer-utils`. |
...
- [x] `HolderSigner` interface and `buildSdJwtVcPresentation()` in `@1shotapi/ows-credentials`
...
| Path | Role |
|------|------|
| `packages/ows-credentials/src/sd-jwt-vc/` | SD-JWT VC issue (mock), present, verify |
| `packages/ows-credentials/src/storage.ts` | `CredentialStore` interface (branding implements) |
