...
import type { Oid4vciClient } from "../oid4vci/client.js";
import type { Oid4vpClient } from "../oid4vp/client.js";
...
import { MOCK_KYC_OFFER_URI, MOCK_KYC_PRESENTATION_URI } from "./fixtures.js";
...
export class DemoCredentialFlow {
  ...
  async acceptOffer(
    ...
    const stored = await this.oid4vci.requestCredential(offer, metadata);
  ...
  async present(
    ...
    return this.oid4vp.buildPresentation(credential, definition);
  }
...
/** MOCK verifier — checks presentation against KycProfilePolicy. */
export function validateMockPresentation(
  ...
  if (!presentation.presentation.startsWith("MOCK_PRESENTATION")) {
    reasons.push("Not a mock presentation blob");
  }
